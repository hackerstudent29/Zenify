...
async function overhaulAesthetics() {
    try {
        ...
        for (const track of tracks) {
            // Target any track that has a generic or missing cover
            if (!track.coverUrl || track.coverUrl.includes('picsum') || track.coverUrl.includes('placeholder')) {
                ...
                await fetch(`${API_BASE}/tracks/${track.id}`, {
                    ...
                    body: JSON.stringify({ coverUrl: randomPhoto })
                });
        ...
        const artistsRes = await fetch(`${API_BASE}/tracks/artists`);
        const artists = await artistsRes.json();
        for (const artist of artists) {
            if (!artist.imageUrl || artist.imageUrl.includes('picsum')) {
                const randomPhoto = AESTHETIC_PHOTOS[Math.floor(Math.random() * AESTHETIC_PHOTOS.length)];
                await fetch(`${API_BASE}/tracks/artists/${artist.id}`, {
                    method: 'PATCH',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ imageUrl: randomPhoto })
                });
