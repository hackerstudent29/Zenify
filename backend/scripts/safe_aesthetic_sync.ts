...
async function main() {
    console.log('🛡️ Executing Safe Aesthetic Sync...');
    console.log('Skipping tracks with custom user-uploaded or externally set images.');
    ...
    for (let i = 0; i < tracks.length; i++) {
        ...
        // Logic to detect if it's a "dummy" or "default" image that needs replacing
        // 1. If coverUrl is empty or null
        // 2. If it's a seed-based picsum/dicebear URL (previous defaults)
        // 3. If it's the generic unsplash backup we used earlier
        const isDefault = !track.coverUrl ||
            track.coverUrl.includes('picsum.photos/seed') ||
            track.coverUrl.includes('api.dicebear.com') ||
            track.coverUrl.includes('images.unsplash.com/photo-1614613535308-eb5fbd3d2c17'); // Previous hardcoded default

        // 4. If it's a user upload (/public/music/...), SAFELY SKIP IT
        const isUserUpload = track.coverUrl?.startsWith('/public/music/');

        if (isDefault && !isUserUpload) {
            let selectedUrl = "";
    ...
    console.log(`🛡️ Tracks Preserved (User/Custom): ${skippedCount}`);
}
