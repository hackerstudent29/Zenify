...
const G = `
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
  ...
  ::-webkit-scrollbar{width:3px;}::-webkit-scrollbar-thumb{background:rgba(139,92,246,0.4);border-radius:3px;}
  ...
  @keyframes pulse2{0%,100%{box-shadow:0 0 0 0 rgba(139,92,246,0.4)}50%{box-shadow:0 0 0 8px rgba(139,92,246,0)}}
...
function GlassInput({ label, placeholder, value, onChange, type = "text", multiline, rows = 4, half, prefix }) {
    ...
    const baseStyle = {
        width: "100%", background: f ? "rgba(139,92,246,0.07)" : "rgba(255,255,255,0.04)",
        border: `1.5px solid ${f ? "rgba(139,92,246,0.6)" : "rgba(255,255,255,0.09)"}`,
        ...
        boxShadow: f ? "0 0 0 4px rgba(139,92,246,0.1)" : "none",
        resize: "none",
    };
    return (
    ...
    return (
        <div style={{ position: "relative" }}>
            ...
            <button onClick={() => setOpen(!open)} style={{
                width: "100%", background: open ? "rgba(139,92,246,0.07)" : "rgba(255,255,255,0.04)",
                border: `1.5px solid ${open ? "rgba(139,92,246,0.6)" : "rgba(255,255,255,0.09)"}`,
                ...
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="rgba(139,92,246,0.7)" strokeWidth="2.5" style={{ transform: open ? "rotate(180deg)" : "none", transition: "0.2s" }}><polyline points="6 9 12 15 18 9" /></svg>
            </button>
            {open && <>
                <div style={{ position: "fixed", inset: 0, zIndex: 90 }} onClick={() => setOpen(false)} />
                <div style={{
                    position: "absolute", top: "calc(100% + 6px)", left: 0, right: 0, zIndex: 91,
                    background: "rgba(14,11,26,0.98)", border: "1.5px solid rgba(139,92,246,0.3)",
                    ...
                    {options.map(o => (
                        <button key={o} onClick={() => { onChange(o); setOpen(false); }} style={{
                            width: "100%", background: value === o ? "rgba(139,92,246,0.15)" : "none", border: "none",
                            borderBottom: "1px solid rgba(255,255,255,0.04)", padding: "12px 16px",
                            color: value === o ? "rgb(167,139,250)" : "rgba(255,255,255,0.6)",
                            fontSize: "0.88rem", fontWeight: 300, textAlign: "left", cursor: "pointer",
                            transition: "all 0.15s",
                        }} onMouseEnter={e => { if (value !== o) { e.target.style.background = "rgba(139,92,246,0.08)"; e.target.style.color = "rgba(255,255,255,0.9)"; } }}
...
function MiniCalendar({ value, onChange, onClose }) {
    ...
    return (<>
        ...
        <div style={{
            ...
            zIndex: 200, background: "rgba(10,8,22,0.98)", border: "1.5px solid rgba(139,92,246,0.3)",
            borderRadius: 20, padding: 20, width: 270, backdropFilter: "blur(20px)",
            boxShadow: "0 30px 80px rgba(0,0,0,0.8),0 0 40px rgba(139,92,246,0.1)", animation: "in 0.18s ease both",
        }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", gap: 2 }}>
                ...
                {Array.from({ length: days }, (_, i) => i + 1).map(d => (
                    <button key={d} onClick={() => pick(d)} style={{
                        background: isSel(d) ? "rgba(139,92,246,1)" : isTod(d) ? "rgba(139,92,246,0.18)" : "none",
                        border: `1px solid ${isSel(d) ? "transparent" : isTod(d) ? "rgba(139,92,246,0.4)" : "transparent"}`,
                        ...
                        boxShadow: isSel(d) ? "0 4px 14px rgba(139,92,246,0.5)" : "none",
                    }} onMouseEnter={e => { if (!isPast(d) && !isSel(d)) { e.target.style.background = "rgba(139,92,246,0.12)"; e.target.style.color = "white"; } }}
                        onMouseLeave={e => { if (!isPast(d) && !isSel(d)) { e.target.style.background = isTod(d) ? "rgba(139,92,246,0.18)" : "none"; e.target.style.color = "rgba(255,255,255,0.7)"; } }}>{d}</button>
                ))}
            </div>
        </div>
    </>);
...
function Pill({ label, active, onClick }) {
    return (
        <button onClick={onClick} style={{
            background: active ? "linear-gradient(135deg,rgba(139,92,246,0.35),rgba(99,52,206,0.25))" : "rgba(255,255,255,0.03)",
            border: `1.5px solid ${active ? "rgba(139,92,246,0.7)" : "rgba(255,255,255,0.08)"}`,
            borderRadius: 100, padding: "9px 20px", fontSize: "0.72rem", fontWeight: 600,
            letterSpacing: "0.1em", textTransform: "uppercase", color: active ? "rgb(196,162,255)" : "rgba(255,255,255,0.35)",
            cursor: "pointer", transition: "all 0.2s",
    ...
    return (
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 0", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
            ...
            <div onClick={() => onChange(!on)} style={{
                ...
                background: on ? "linear-gradient(135deg,#8b5cf6,#6d28d9)" : "rgba(255,255,255,0.07)",
                border: `1.5px solid ${on ? "transparent" : "rgba(255,255,255,0.1)"}`,
                transition: "all 0.3s", boxShadow: on ? "0 4px 16px rgba(139,92,246,0.45)" : "none",
            }}>
                ...
                return (<>
                    <div key={s} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6, cursor: done ? "pointer" : "default" }}>
                        <div style={{
                            ...
                            background: active ? "linear-gradient(135deg,#8b5cf6,#6d28d9)" : done ? "rgba(139,92,246,0.2)" : "rgba(255,255,255,0.05)",
                            border: `2px solid ${active ? "#8b5cf6" : done ? "rgba(139,92,246,0.5)" : "rgba(255,255,255,0.1)"}`,
                            transition: "all 0.3s", boxShadow: active ? "0 0 20px rgba(139,92,246,0.5)" : "none",
                            animation: active ? "pulse2 2s infinite" : "none",
                        }}>
                            {done
                                ? <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="rgba(167,139,250,0.9)" strokeWidth="3"><polyline points="20 6 9 17 4 12" /></svg>
                                : <span style={{ fontSize: "0.65rem", fontWeight: 700, color: active ? "white" : "rgba(255,255,255,0.25)", letterSpacing: "0" }}>{i + 1}</span>
                            }
                        </div>
                        <span style={{ fontSize: "0.55rem", fontWeight: 600, letterSpacing: "0.12em", textTransform: "uppercase", color: active ? "rgba(196,162,255,0.9)" : done ? "rgba(139,92,246,0.6)" : "rgba(255,255,255,0.2)", whiteSpace: "nowrap" }}>{s}</span>
                    </div>
                    {i < steps.length - 1 && <div style={{ flex: 1, height: 1.5, background: i < current ? "rgba(139,92,246,0.4)" : "rgba(255,255,255,0.06)", margin: "0 10px", marginBottom: 22, transition: "all 0.4s" }} />}
                </>);
            })}
        </div>
    );
        ...
        <div className="step-enter" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }}>
            ...
            <div>
                ...
                <div onClick={() => covRef.current.click()} style={{
                    ...
                    border: `2px dashed ${coverPreview ? "transparent" : "rgba(139,92,246,0.25)"}`,
                    display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center",
                    transition: "all 0.3s",
                }} onMouseEnter={e => { if (!coverPreview) e.currentTarget.style.borderColor = "rgba(139,92,246,0.5)"; e.currentTarget.style.background = "rgba(139,92,246,0.04)"; }}
                    onMouseLeave={e => { if (!coverPreview) e.currentTarget.style.borderColor = "rgba(139,92,246,0.25)"; e.currentTarget.style.background = coverPreview ? "none" : "rgba(255,255,255,0.02)"; }}>
                    ...
                    {coverPreview
                        ...
                        : <>
                            <div style={{ width: 56, height: 56, borderRadius: 16, background: "rgba(139,92,246,0.12)", border: "1.5px solid rgba(139,92,246,0.25)", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 14 }}>
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="rgba(167,139,250,0.7)" strokeWidth="1.5"><rect x="3" y="3" width="18" height="18" rx="3" /><circle cx="8.5" cy="8.5" r="1.5" /><polyline points="21 15 16 10 5 21" /></svg>
                            </div>
            ...
            <div>
                ...
                <div onDragOver={e => { e.preventDefault(); setDrag(true); }} onDragLeave={() => setDrag(false)}
                    ...
                    onClick={() => audRef.current.click()} style={{
                        height: 220, borderRadius: 20, border: `2px dashed ${drag ? "rgba(139,92,246,0.7)" : "rgba(255,255,255,0.09)"}`,
                        background: drag ? "rgba(139,92,246,0.06)" : audioFile ? "rgba(139,92,246,0.04)" : "rgba(255,255,255,0.02)",
                        display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center",
                    ...
                    {audioFile
                        ? <>
                            <div style={{ width: 52, height: 52, borderRadius: 16, background: "rgba(139,92,246,0.2)", border: "1.5px solid rgba(139,92,246,0.4)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                                <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="rgba(167,139,250,0.9)" strokeWidth="1.5"><path d="M9 18V5l12-2v13M9 18c0 1.1-.9 2-2 2s-2-.9-2-2 .9-2 2-2 2 .9 2 2zm12 0c0 1.1-.9 2-2 2s-2-.9-2-2 .9-2 2-2 2 .9 2 2z" /></svg>
                            </div>
                            <div style={{ textAlign: "center", padding: "0 16px" }}>
                                <div style={{ fontSize: "0.82rem", fontWeight: 500, color: "rgba(196,162,255,0.9)", marginBottom: 4, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: 200 }}>{audioName}</div>
                            ...
                            <div style={{ display: "flex", alignItems: "flex-end", gap: 3, height: 32, padding: "0 8px" }}>
                                {Array.from({ length: 24 }, (_, i) => (
                                    <div key={i} style={{
                                        width: 3, borderRadius: 2, background: `rgba(139,92,246,${0.4 + Math.random() * 0.5})`,
                ...
                <div style={{ marginTop: 16, background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 14, padding: "14px 16px" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
                        ...
                        <span style={{ fontSize: "0.72rem", fontWeight: 600, color: "rgba(167,139,250,0.85)" }}>99.9%</span>
                    </div>
                    <div style={{ height: 3, borderRadius: 100, background: "rgba(255,255,255,0.06)", overflow: "hidden" }}>
                        <div style={{ height: "100%", width: "99.9%", borderRadius: 100, background: "linear-gradient(90deg,#6d28d9,#8b5cf6,#a78bfa)", boxShadow: "0 0 10px rgba(139,92,246,0.5)" }} />
                    </div>
                </div>
            </div>
        </div>
    ...
    const RelCard = ({ value, label, sub, icon }) => {
        ...
        return (
            <div onClick={() => setMode(value)} style={{
                ...
                boxShadow: active ? "inset 0 0 30px rgba(139,92,246,0.05),0 0 0 1px rgba(139,92,246,0.1)" : "none",
                display: "flex", alignItems: "center", justifyContent: "space-between",
            }}>
                <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                    <div style={{ width: 38, height: 38, borderRadius: 12, background: active ? "rgba(139,92,246,0.2)" : "rgba(255,255,255,0.04)", border: `1px solid ${active ? "rgba(139,92,246,0.4)" : "rgba(255,255,255,0.08)"}`, display: "flex", alignItems: "center", justifyContent: "center", color: active ? "rgb(167,139,250)" : "rgba(255,255,255,0.25)", flexShrink: 0, transition: "all 0.2s" }}>{icon}</div>
                    <div>
                ...
                <div style={{ width: 18, height: 18, borderRadius: "50%", border: `2px solid ${active ? "#8b5cf6" : "rgba(255,255,255,0.15)"}`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                    {active && <div style={{ width: 8, height: 8, borderRadius: "50%", background: "#8b5cf6" }} />}
                </div>
            </div>
        );
    };
        ...
        <div className="step-enter" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 28 }}>
                    ...
                    <div style={{ marginTop: 16, display: "flex", flexDirection: "column", gap: 12, animation: "in 0.2s ease both" }}>
                        <div style={{ position: "relative" }}>
                            ...
                            <button onClick={() => setShowCal(!showCal)} style={{
                                width: "100%", background: "rgba(255,255,255,0.04)", border: `1.5px solid ${showCal ? "rgba(139,92,246,0.6)" : "rgba(255,255,255,0.09)"}`,
                                ...
                                boxShadow: showCal ? "0 0 0 4px rgba(139,92,246,0.1)" : "none", transition: "all 0.2s",
                            }}>
                                <span>{schedDate || "Select date…"}</span>
                                <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="rgba(139,92,246,0.6)" strokeWidth="2"><rect x="3" y="4" width="18" height="18" rx="2" /><line x1="16" y1="2" x2="16" y2="6" /><line x1="8" y1="2" x2="8" y2="6" /><line x1="3" y1="10" x2="21" y2="10" /></svg>
                            </button>
                            {showCal && <MiniCalendar value={schedDate} onChange={setSchedDate} onClose={() => setShowCal(false)} />}
                        </div>
                        <CustomSelect label="Release Time" value={schedTime} onChange={setSchedTime} options={TIMES} placeholder="Select time…" />
                    </div>
                )}
            </div>
            ...
            <div>
                ...
                <div style={{ marginTop: 20 }}>
                    ...
                    <div style={{ position: "relative" }}>
                        <span style={{ position: "absolute", left: 14, top: "50%", transform: "translateY(-50%)", color: "rgba(139,92,246,0.7)", fontSize: "0.9rem", pointerEvents: "none", zIndex: 1 }}>@</span>
                        <input value={copyright} onChange={e => setCopyright(e.target.value)} placeholder="Label name (e.g. ram)"
                            style={{ width: "100%", background: "rgba(255,255,255,0.04)", border: "1.5px solid rgba(139,92,246,0.35)", borderRadius: 14, padding: "13px 16px 13px 34px", color: "rgba(255,255,255,0.9)", fontSize: "0.88rem", fontWeight: 300, outline: "none", transition: "all 0.2s", boxShadow: "0 0 0 4px rgba(139,92,246,0.06)" }}
                            onFocus={e => { e.target.style.borderColor = "rgba(139,92,246,0.65)"; e.target.style.boxShadow = "0 0 0 4px rgba(139,92,246,0.12)"; }}
                            onBlur={e => { e.target.style.borderColor = "rgba(139,92,246,0.35)"; e.target.style.boxShadow = "0 0 0 4px rgba(139,92,246,0.06)"; }} />
                    </div>
                    {cidDisplay && (
                        <div style={{ marginTop: 8, display: "inline-flex", alignItems: "center", gap: 6, background: "rgba(139,92,246,0.12)", border: "1px solid rgba(139,92,246,0.25)", borderRadius: 8, padding: "5px 12px", animation: "in 0.2s ease both" }}>
                            <div style={{ width: 6, height: 6, borderRadius: "50%", background: "rgba(167,139,250,0.8)", animation: "pulse2 2s infinite" }} />
                            <span style={{ fontSize: "0.76rem", fontWeight: 500, color: "rgba(196,162,255,0.9)" }}>@{cidDisplay}</span>
                        </div>
                    )}
    ...
    return (
        <div className="step-enter" style={{ display: "grid", gridTemplateColumns: "200px 1fr", gap: 28 }}>
            ...
            <div>
                ...
                {audioName && <div style={{ marginTop: 10, background: "rgba(139,92,246,0.08)", border: "1px solid rgba(139,92,246,0.2)", borderRadius: 10, padding: "8px 12px" }}>
                    <div style={{ fontSize: "0.58rem", fontWeight: 600, letterSpacing: "0.1em", textTransform: "uppercase", color: "rgba(139,92,246,0.6)", marginBottom: 2 }}>Audio</div>
            ...
            <div>
                <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 16, padding: "6px 20px", marginBottom: 20 }}>
                    ...
                    <Row label="Copyright" value={copyright ? `@${copyright.replace(/\s+/g, "").toLowerCase()}` : null} />
                </div>
                ...
                <div onClick={() => setCertified(!certified)} style={{
                    ...
                    <div style={{ width: 20, height: 20, borderRadius: 6, border: `2px solid ${certified ? "#8b5cf6" : "rgba(255,255,255,0.15)"}`, background: certified ? "rgba(139,92,246,0.2)" : "none", display: "flex", alignItems: "center", justifyContent: "center", transition: "all 0.2s", flexShrink: 0, marginTop: 1 }}>
                        {certified && <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#a78bfa" strokeWidth="3"><polyline points="20 6 9 17 4 12" /></svg>}
                    </div>
                    <div>
                ...
                <button onClick={onCommit} disabled={!certified} style={{
                    ...
                    background: certified ? "linear-gradient(135deg,#8b5cf6 0%,#6d28d9 100%)" : "rgba(255,255,255,0.05)",
                    ...
                    boxShadow: certified ? "0 8px 30px rgba(139,92,246,0.4)" : "none",
                }} onMouseEnter={e => { if (certified) { e.target.style.transform = "translateY(-2px)"; e.target.style.boxShadow = "0 14px 40px rgba(139,92,246,0.5)"; } }}
                    onMouseLeave={e => { if (certified) { e.target.style.transform = "none"; e.target.style.boxShadow = "0 8px 30px rgba(139,92,246,0.4)"; } }}
                >
                    Commit Release
                </button>
            </div>
...
function SectionLabel({ num, title, sub }) {
    return (
        <div style={{ marginBottom: 20, display: "flex", alignItems: "center", gap: 12 }}>
            <div style={{ width: 30, height: 30, borderRadius: 10, background: "rgba(139,92,246,0.15)", border: "1.5px solid rgba(139,92,246,0.3)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "0.65rem", fontWeight: 700, color: "rgba(167,139,250,0.9)", flexShrink: 0 }}>{num}</div>
            <div>
    ...
    if (committed) return (
        <div style={{ minHeight: "100vh", background: "#06050c", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", fontFamily: "'Sora',sans-serif" }}>
            ...
            <div style={{ textAlign: "center", animation: "in 0.5s ease both" }}>
                <div style={{ width: 72, height: 72, borderRadius: "50%", background: "linear-gradient(135deg,#8b5cf6,#6d28d9)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 24px", boxShadow: "0 0 40px rgba(139,92,246,0.5)" }}>
                ...
                <button onClick={() => { setCommitted(false); setStep(0); }} style={{ marginTop: 32, background: "rgba(139,92,246,0.15)", border: "1.5px solid rgba(139,92,246,0.35)", borderRadius: 12, color: "rgba(196,162,255,0.9)", fontSize: "0.76rem", fontWeight: 600, letterSpacing: "0.12em", textTransform: "uppercase", padding: "12px 28px", cursor: "pointer" }}>Upload Another</button>
            </div>
        </div>
    );

    return (
        <div style={{ minHeight: "100vh", background: "#06050c", backgroundImage: "radial-gradient(ellipse 70% 40% at 60% 0%,rgba(109,40,217,0.08) 0%,transparent 55%)", fontFamily: "'Sora',sans-serif", color: "rgba(255,255,255,0.85)" }}>
            <style>{G}</style>
            ...
            <div style={{ borderBottom: "1px solid rgba(255,255,255,0.06)", background: "rgba(6,5,12,0.85)", backdropFilter: "blur(20px)", padding: "0 48px", height: 60, display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 50 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    <div style={{ width: 28, height: 28, borderRadius: 8, background: "linear-gradient(135deg,#8b5cf6,#6d28d9)", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 4px 12px rgba(139,92,246,0.4)" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    ...
                    <div style={{ width: 32, height: 32, borderRadius: "50%", background: "linear-gradient(135deg,#8b5cf6,#6d28d9)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "0.65rem", fontWeight: 700, boxShadow: "0 4px 12px rgba(139,92,246,0.3)" }}>ZK</div>
                </div>
            </div>
            ...
            <div style={{ maxWidth: 900, margin: "0 auto", padding: "48px 48px 80px" }}>
                ...
                <div style={{ marginBottom: 44 }}>
                    <div style={{ fontSize: "0.58rem", fontWeight: 600, letterSpacing: "0.22em", textTransform: "uppercase", color: "rgba(139,92,246,0.6)", marginBottom: 10 }}>Upload Progress — Step {step + 1}</div>
                ...
                {step < 3 && (
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 40, paddingTop: 28, borderTop: "1px solid rgba(255,255,255,0.06)" }}>
                        <button onClick={() => setStep(s => Math.max(0, s - 1))} disabled={step === 0} style={{ background: "none", border: "1.5px solid rgba(255,255,255,0.1)", borderRadius: 12, color: step === 0 ? "rgba(255,255,255,0.15)" : "rgba(255,255,255,0.5)", fontSize: "0.75rem", fontWeight: 600, letterSpacing: "0.12em", textTransform: "uppercase", padding: "12px 28px", cursor: step === 0 ? "default" : "pointer", transition: "all 0.2s" }}
                            ...
                            onMouseLeave={e => { e.target.style.borderColor = "rgba(255,255,255,0.1)"; }}>← Back</button>

                        <div style={{ fontSize: "0.6rem", color: "rgba(255,255,255,0.2)", letterSpacing: "0.1em" }}>
                            {!canNext[step] && ["Add an audio file to continue", "Fill artist, title & genre", "", "Check certification"][step]}
                        </div>

                        <button onClick={() => setStep(s => Math.min(3, s + 1))} disabled={!canNext[step]} style={{
                            background: canNext[step] ? "linear-gradient(135deg,#8b5cf6,#6d28d9)" : "rgba(255,255,255,0.05)",
                            ...
                            boxShadow: canNext[step] ? "0 6px 24px rgba(139,92,246,0.4)" : "none",
                        }} onMouseEnter={e => { if (canNext[step]) { e.target.style.transform = "translateY(-1px)"; e.target.style.boxShadow = "0 10px 30px rgba(139,92,246,0.5)"; } }}
                            onMouseLeave={e => { e.target.style.transform = "none"; e.target.style.boxShadow = canNext[step] ? "0 6px 24px rgba(139,92,246,0.4)" : "none"; }}>
