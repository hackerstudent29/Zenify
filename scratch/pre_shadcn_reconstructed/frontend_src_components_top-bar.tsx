      ...
      <div className="flex items-center gap-4 shrink-0">
        ...
        {!isMobile && currentTrack && (
          <div className="flex items-center gap-3 px-3 py-1.5 bg-zinc-900/60 rounded-full border border-white/10 shadow-2xl transition-all duration-300 select-none">
            ...
            <div className="flex flex-col max-w-[140px] pr-2 text-left select-none">
              <span 
                onClick={(e) => { e.stopPropagation(); router.push(`/track/${currentTrack.id}`); }}
                className="font-brand text-[10.5px] truncate leading-none text-foreground tracking-tight hover:text-brand cursor-pointer transition-colors block"
              >
                {currentTrack.title}
              </span>
              {currentTrack.artist?.id ? (
                <span 
                  ...
                  className="text-[10px] text-zinc-500 truncate mt-0.5 font-medium hover:text-brand cursor-pointer transition-colors block"
                >
                  {currentTrack.artist?.name || 'Unknown Artist'}
                </span>
              ) : (
                <span className="text-[10px] text-zinc-500 truncate mt-0.5 font-medium block">
                  {currentTrack.artist?.name || 'Unknown Artist'}
                </span>
              )}
            </div>
